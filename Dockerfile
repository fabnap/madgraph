...
# 2) Download & unpack MG5_aMC
WORKDIR /opt
ENV MG5_VERSION=3.5.3
RUN wget https://launchpad.net/mg5amcnlo/3.0/${MG5_VERSION}.x/+download/MG5_aMC_v${MG5_VERSION}.tar.gz && \
    tar -xzf MG5_aMC_v${MG5_VERSION}.tar.gz && \
    rm MG5_aMC_v${MG5_VERSION}.tar.gz

# 3) Install Pythia8 & Delphes via MG5 installer
WORKDIR /opt/MG5_aMC_v3_5_3
RUN printf "install pythia8\ninstall Delphes\n" > install_script && \
    ./bin/mg5_aMC install_script && \
    rm install_script

# 4) Verify installation (optional smoke‐test)
RUN ./bin/mg5_aMC --help | head -n 5

# 5) Expose MG5 in PATH
ENV PATH="/opt/MG5_aMC_v3_5_3/bin:$PATH"

# 6) Make a dedicated Jupyter kernel
